...
def flatten_X(X, task):
    if task =='AF':
      assert X.shape[1] == 6, "X channel number error"
      assert X.shape[2] == 100, "X timespan length error"
    elif task == 'MMA':
      assert X.shape[1] == 6, "X channel number error"
      assert X.shape[2] == 81, "X timespan length error"
    return X.reshape(X.shape[0],X.shape[1]*X.shape[2])

# Profile shifting and padding
# Input: X = (N, timespan, channel) signal inputs;
# padded_length = timespan after shifting and padding
# Output: X with timespan = 201, maxpoint at 100 ms
