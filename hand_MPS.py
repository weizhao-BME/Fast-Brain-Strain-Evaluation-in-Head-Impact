...
def train_augment(trainX, trainY):
  ...
  for id in range(0,row):
      ...
      #add_03[id,:] = 0.04 * standard_deviation * np.random.randn(1, column)
...
for repeat in range(0,10):
  ...
  MMA_Y = io.loadmat("MMA_Y.mat")["label_data"]
  AF_X = io.loadmat("AF_X_hand.mat")["AF_X"]
  X = np.row_stack((MMA_X,AF_X))
  print('X shape: {}'.format(np.shape(X)))
  AF_Y = io.loadmat("AF_Y.mat")["AF_Y"]
  if AF_Y.shape[0]==518:
      AF_Y = np.delete(AF_Y, [351], axis=0)
  print(np.shape(MMA_Y))
  Y = np.row_stack((MMA_Y, AF_Y))
  ...
  input_nodes = 36
  first_hidden_layer = 120 #60/50/45/40/35/30/25
  second_hidden_layer = 480 #40/35/30/25
  lr = 0.0007
  third_hidden_layer = 1000 #25
...
#predict_result = pd.DataFrame({"Predicted MPS": predicted_MPS})
  prediction_path = "C:\\Users\\zhanx\\OneDrive\\Desktop\\Stanford\\Deep Learning\\Project 2 Data\\Project2 Data\\Project\\Prediction_ANN_" + 'repeat' + str(repeat)+ '_' + augment + '_' + 'Dropout0.5_2_'+ str(standardization) + "_"+task + "_" + str(first_hidden_layer)+"_"+str(second_hidden_layer)+"_"+str(third_hidden_layer) +"_"+str(output_nodes)+ "_" + \
                  'lr' + str(lr) +"_"+"epoch"+str(epoch) + "_regularization" + str(regularization) + "_initialization" + str(initialization) + "_" + str(loss) + '.mat'
#predict_result.to_csv(prediction_path, index=True, sep=',')

  io.savemat(prediction_path,{'Prediction':predicted_Y, 'Y_test': Y_test,'Prediction Val': predicted_Y_val, 'Y_val': Y_val, 'Train Time': train_time,
...
# serialize model to JSON
  ...
  model_path = "DNN model\\DNN_"+ 'repeat' + str(repeat)+ '_' + augment+ '_' + 'Dropout0.5_2_'+ str(standardization) + "_"+ task +"_"+ str(first_hidden_layer)+"_"+str(second_hidden_layer)+"_"+str(third_hidden_layer)+"_"+str(output_nodes)+ "_" +'lr' + str(lr) +"_"+"epoch"+str(epoch) + "_regularization" + str(regularization) + "_initialization" + str(initialization) + "_" + str(loss)+"_model.json"
  with open(model_path, "w") as json_file:  #Save model skeleton
    json_file.write(model_json)
# serialize weights to HDF5
  weight_path = "DNN model\\ANN_"+ 'repeat' + str(repeat)+ '_' + augment+ '_' + 'Dropout0.5_2_'+ str(standardization) + "_" + task +  "_" + str(first_hidden_layer)+"_"+str(second_hidden_layer)+"_"+str(third_hidden_layer)+"_"+str(output_nodes)+ "_" +'lr' + str(lr) +"_"+"epoch"+str(epoch)+ "_regularization" + str(regularization) + "_initialization" + str(initialization)+ "_" + str(loss)+"_weight.h5"
